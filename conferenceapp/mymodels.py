...
#helper table to show the many to many relationship between two classes
...
class User(db.Model): 
    ...
    #set up relationship between User and Breakout, then specify that we are using an association Table
...
class Contactus(db.Model):
    ...
    contactus_message = db.Column(db.Text(255), nullable=False)
...
class Posts(db.Model):
    ...
    post_comments = db.relationship('Comments', back_populates="postdeets")

class Comments(db.Model):
    __tablename__ = 'comments'
    c_id = db.Column(db.Integer(), primary_key=True, autoincrement=True)
    c_userid = db.Column(db.Integer(), db.ForeignKey("user.user_id"))
    c_postid = db.Column(db.Integer(), db.ForeignKey("posts.p_id"))
    ...
    postdeets = db.relationship('Posts', back_populates="post_comments")